...
if st.sidebar.button("Prédire"):
    ...
    if response.status_code == 200 or response.status_code == 201:
        ...
        st.markdown(f"<div style='background-color: lightgray; padding: 10px;'><b>Client N° =</b> <span style='font-size: 1.2em;'>{selected_client}</span> {client}</div>", unsafe_allow_html=True)

        st.write("\n")
               
        # df_cam= df[df["SK_ID_CURR"]==selected_client]
        ...
        # Ajout d'un titre
        ax.set_title('Répartition Clients acceptés vs refusés sur ce filtre')
